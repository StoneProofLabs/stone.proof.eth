...
export default function InspectorDetailsPage() {
  ...
  useEffect(() => {
    router.replace(`/admin/inspectors/${id}?status=${status}`);
  }, [status]);

  if (!inspector) {
    return <div className="text-white p-10 text-center text-2xl">Inspector not found</div>;
  }

  // Mocked values for demo (replace with real data as needed)
  const inspectionProgress = 18;
  const quantity = "20 Tons";
  const totalPendings = "1,234";
  const disputes = 50;
  const topDelay = "3 Days";
  const inspectorAddress = "45 Inspector Lane";
  ...
  return (
    <div className="min-h-screen text-white p-4 md:p-10 flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0">
        <div className="w-full flex flex-row items-center justify-between">
          <h1 className="text-xl sm:text-3xl md:text-5xl font-semibold leading-tight">
            Inspector _ID: <span className="text-white">{inspector.code}</span>
          </h1>
          <span
          ...
          <div className="flex flex-col gap-4 md:gap-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full mb-4 gap-2">
              <div className="flex items-center gap-3">
                ...
                />
                <span className="font-bold text-lg md:text-xl">{inspector.name}</span>
              </div>
              <span className="text-gray-400 text-sm md:text-base">{inspector.registeredOn} - Present</span>
            </div>
            <div className="flex flex-col md:flex-row gap-4 md:gap-6">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="w-full h-3 bg-white rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500" style={{ width: `${inspectionProgress}%` }}></div>
                    </div>
              ...
              <div className="flex-1 flex flex-col gap-4 md:gap-5">
                <div>
                  <span className="block text-white font-bold mb-2">INSPECTOR_ID</span>
                  <div className="border border-[#1E2328] rounded px-4 py-3 flex items-center justify-between">
                    <span className="font-mono text-white text-base">{inspector.code}</span>
                ...
                <div>
                  <span className="block text-white font-bold mb-2">Inspector Address</span>
                  <div className="border border-[#1E2328] rounded px-4 py-3 text-white">{inspectorAddress}</div>
                </div>
                <div>
                  <span className="block text-white font-bold mb-2">Inspector Name</span>
                  <div className="border border-[#1E2328] rounded px-4 py-3 text-white">{inspectorName}</div>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col items-start bg-transparent w-full md:w-auto">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400">12.3kb</div>
                  </div>
                </div>
      ...
      <p className="text-gray-400 text-md mt-2">
        If an inspection is revoked he/she won&apos;t be able to access the system!
      </p>
    </div>
  );
}
