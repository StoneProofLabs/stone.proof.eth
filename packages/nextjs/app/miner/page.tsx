...
import { ShieldAlert, Copy, Mail, Phone, MessageSquare, ChevronRight, HardHat } from "lucide-react";
...
const AccessDeniedCard = ({ address }: { address: string }) => {
  const copyAddress = () => {
    ...
    <div className="max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
      <div className="text-center space-y-4">
        ...
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
          Miner Privileges Required
        </h2>

        <p className="text-gray-600 dark:text-gray-300">
          Your wallet doesn't have miner access permissions.
        </p>
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-green-200 dark:border-green-800">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-600 dark:text-gray-300">
          You have full access to the miner portal.
        </p>

        <button
          onClick={() => window.location.href = "/miner/overview"}
...
export default function MinerPortalEntry() {
  ...
  const [status, setStatus] = useState<"checking" | "no-access" | "access-granted">("checking");

  const { data: hasMinerRole, isLoading } = useScaffoldReadContract({
  ...
  useEffect(() => {
    ...
    if (!isLoading) {
      setStatus(hasMinerRole ? "access-granted" : "no-access");
    }
  }, [isConnected, isLoading, hasMinerRole]);
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4">
      {status === "no-access" && address && <AccessDeniedCard address={address} />}
      {status === "access-granted" && <MinerAccessGranted />}
    </div>
  );
}